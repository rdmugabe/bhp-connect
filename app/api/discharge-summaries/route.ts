...
export async function POST(request: NextRequest) {
  try {
    ...
    const parseResult = await parseJsonBody<{ intakeId?: string; isDraft?: boolean }>(request);
    ...
    const dischargeSummary = await prisma.dischargeSummary.create({
      data: {
        intakeId,
        facilityId: bhrfProfile.facilityId,
        dischargeDate: validatedData.dischargeDate ? new Date(validatedData.dischargeDate) : new Date(),
